...
public interface TagRepository extends JpaRepository<Tag, Long> {
    ...
    @Query(
        "SELECT count(t) AS tagCount, t.id AS tagId FROM tag t "
            ...
            + "LEFT JOIN gift_certificate.receipts r "
            + "WHERE r.totalCost = (SELECT max(r.totalCost) FROM receipt r WHERE r.person.id = :id) "
            + "AND r.person.id = :id GROUP BY t.id")
    List<TagsCountToTagsIdProjection> findCountTagsInPersonMaxCostReceipt(Long id);
}
